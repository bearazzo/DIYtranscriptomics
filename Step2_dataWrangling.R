# Introduction to the Step 2 script ---- DATA NORMALIZATION
...
library(edgar) # well known package for differential expression analysis, but we only use for the DGEList object and for normalization methods
...
colSums(myTPM) #Gene level data does not fully add up to 1 million in a way that transcript level data would have
colSums(myCounts)

# capture sample labels from the study design file that you worked with and saved as 'targets' in step 1
#Always have a good study design file with as much information as is practical
targets
...
ggplot(myTPM.stats) + 
  ...
  geom_hex(show.legend = FALSE) + #summarizes data in hexagonal points
...
colnames(log2.cpm.df) <- c("geneID", sampleLabels) #Tibble counts row names as first column
...
log2.cpm.df.pivot #Why am I not getting 10-fold more? 

# not it is easy to plot this pivoted data - DISTRIBUTION OF ALL GENES - STRANGE WAY TO LOOK
...
table(rowSums(myDGEList$counts==0)==10) #this is kinda difficult syntax
...
keepers <- rowSums(cpm>1)>=5 #Filtering out genes without at least 1cpm in 
...
# Normalize your data ---- there is a paper that discusses this method -- I THINK THIS IS THE MOST HELPFUL THING FOR YOU TO GO OVER.
