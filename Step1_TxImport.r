...
setwd("~/Documents/DIYTranscriptomics/MappedReads")

# load packages----
setRepositories() #Select all in console
install.packages("rhdf5")
install.packages("tximport")
#Missing dependencies: Rsamtools, rtracklayer, GenomicAlignments 
if (!require("BiocManager", quietly = TRUE))
  install.packages("BiocManager")
BiocManager::install("Rsamtools")
BiocManager::install("rtracklayer") #ok. Missing dependencies: rgdal, maptools 
install.packages("ensembldb") #ok. Missing dependencies: rtracklayer, GenomicAlignments 
install.packages("EnsDb.Hsapiens.v86")
install.packages("datapasta")

library(GenomicAlignments) #ok
library(rgdal) #ok
library(rtracklayer) #I think ok?
library(Rsamtools) #ok 
library(rhdf5) #ok. provides functions for handling hdf5 file formats (kallisto outputs bootstraps in this format)
library(tidyverse) # provides access to Hadley Wickham's collection of R packages for data science, which we will use throughout the course
library(tximport) #ok. package for getting Kallisto results into R
library(ensembldb) #helps deal with ensembl
library(EnsDb.Hsapiens.v86) #replace with your organism-specific database package
library(beepr) #ok. just for fun
library(datapasta)
library(tibble)
library(dplyr)
